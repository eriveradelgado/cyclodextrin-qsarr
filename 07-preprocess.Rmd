...
## Final NA replacement

After the above cleaning steps, the mean of each descriptor should be 0. The NAs in the data frame will be replaced with 0, as per the assumption that missing values should be assumed to be the expectation of the descriptor. 

```{r}
cdk_list <- lapply(
  cdk_list, 
  function(x) {
    replace_na(x, list(rep(0, ncol(x))))
  }
)
```
